...
const DEFAULT_PROMPT = 'Esta mensagem esta relaciona a algum tipo de ajuda com servidor de fivem QB/QBOX/MRI ou de ajuda em algum script? Analise o conteúdo da mensagem e o nome do canal. Responda APENAS \'sim\' ou \'não\', sem explicações.';
...
async function validateMonitoredChannel(message) {
    if (!message.guild) return false;
    const monitoredChannelId = await getMonitoredChannelId(message.client, message.guild.id);
 ...
 */
async function sendNotificationMessage(originalMessage, notificationMessage, timeout) {
    try {
        const sentMessage = await originalMessage.channel.send(`${originalMessage.author} ${notificationMessage}`);
        
        if (timeout > 0) {
            setTimeout(() => {
                sentMessage.delete().catch(error => {
                    console.error('[Gemini Moderator] Erro ao remover mensagem de notificação:', error);
                });
            }, timeout * 1000);
        }
    } catch (error) {
        console.error('[Gemini Moderator] Erro ao enviar mensagem de notificação:', error);
    }
}
...
async function onMessageCreate(message) {
    try {
        ...
        try {
            ...
            notificationMessage = await getNotificationMessage(message.client, guildId);
            messageTimeout = await getMessageTimeout(message.client, guildId);
            apiKey = process.env.AI_TOKEN;
        } catch (error) {
            console.error(`Erro ao obter configurações do Gemini Moderator para o servidor ${guildId}:`, error);
            return;
        }

        if (!targetChannelId) {
            console.warn(`[Gemini Moderator] Canal de ajuda não configurado para o servidor ${guildId}`);
            return;
        }
        ...
        if (shouldDelete === null) {
            console.error(`[Gemini Moderator] Erro ao verificar mensagem com Gemini API para o servidor ${guildId}`);
            return;
        }
        ...
        if (shouldDelete === true) {
            try {
                ...
                console.log(`[Gemini Moderator] Mensagem deletada no servidor ${guildId}, canal ${channelName}, autor ${message.author.id}`);
            } catch (error) {
                console.error(`[Gemini Moderator] Erro ao deletar mensagem no servidor ${guildId}:`, error);
                return;
            }


            const finalMessage = replaceTargetChannel(notificationMessage, targetChannelId);

            await sendNotificationMessage(message, finalMessage, messageTimeout);
        }
    } catch (error) {
        console.error('[Gemini Moderator] Erro não tratado no handler de mensagens:', error);
    }
}
